...
class PcaBiasCalculator:
    def __init__(
        ...
        biased_word_pairs=sentiment_word_pairs, #gender_biased_word_pairs,
...
if __name__ == "__main__":
    ...
    if not sentence:
        raise ValueError("You must provice a sentence param")
    if len(sentence) > 500:
        raise ValueError("Sentence must be less than 500 characters long")
    
    objs = parse_sentence(sentence)
    results = {}
    for obj in objs:
        results[obj["text"]] = calculator.detect_bias(obj["text"])
